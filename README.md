...
- **Download all files as ZIP**: Use `Ctrl+Shift+A` / `Cmd+Shift+A` or right-click the extension icon to download the entire project as a structured ZIP archive
...
- **Keyboard shortcuts**: Quick access with `Ctrl+Shift+D` (file), `Ctrl+Shift+T` (tree structure), `Ctrl+Shift+A` (all files as ZIP)
...
- **Download all files as ZIP**: `Ctrl+Shift+A` (Windows/Linux) or `Cmd+Shift+A` (Mac)
...
- The extension will automatically expand all folders, open each file, extract its content, and package everything into a structured ZIP

**Method 2: Right-click extension icon**
1. Right-click the Vibe PowerApps extension icon in your Chrome toolbar
2. Select "Download all files as ZIP"
3. Wait for the process to complete — files will be marked with green checkmarks as they are processed
4. A ZIP file will download with format: `vibe.powerapps.[plan-id].zip`

**How it works:**
1. Auto-expands all collapsed folders in the file tree
2. Collects all file paths with their complete directory structure
3. Clicks each file in the tree to open it in the Monaco editor
4. Extracts the full source code content
5. Packages everything into a ZIP with the original folder hierarchy preserved
6. Downloads the ZIP file automatically

**Note:** For large projects (500+ files), the process may take several minutes. Do not switch tabs during the download.
...
1. Extension auto-expands all collapsed folders in the file tree
2. Collects all file paths with their complete directory structure
3. Clicks each file node to open it in the Monaco editor
4. Waits for Monaco to render the file content
5. Extracts source code using the same extraction engine as single-file downloads
6. Packages all files into a ZIP archive with `JSZip` (DEFLATE compression), preserving the original folder hierarchy
7. Downloads the ZIP file automatically via blob URL
